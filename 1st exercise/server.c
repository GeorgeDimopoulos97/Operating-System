...
pthread_mutex_t for_queue_producer = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t for_queue_consumer = PTHREAD_MUTEX_INITIALIZER;
...
void *consumers()
{
 ...
 {
  Queue dim3;
  pthread_mutex_lock(&for_queue_consumer);
  while(count==0)
  {
    pthread_cond_wait(&non_empty_queue,&for_queue_consumer);
  }
  dim3=get();
  pthread_cond_signal(&non_full_queue);
  pthread_mutex_unlock(&for_queue_consumer);

  gettimeofday(&anamoni,NULL);
  process_request(dim3.fd);
  gettimeofday(&support,NULL);
  pthread_mutex_lock(&for_var);
  total_waiting_time=total_waiting_time+((anamoni.tv_sec - dim3.geo.tv_sec)*1000000 + anamoni.tv_usec - dim3.geo.tv_usec);
  total_service_time=total_service_time+((support.tv_sec - anamoni.tv_sec)*1000000 + support.tv_usec - anamoni.tv_usec);
  completed_request++;
...
int main() {
  ...
  while (1) { 
    ...
    Queue dim2;
    dim2.fd=new_fd;
    pthread_mutex_lock(&for_queue_producer); //klidono nimata
    while(count>=QUEUE_SIZE)
    {
      pthread_cond_wait(&non_full_queue,&for_queue_producer); //koimate
    }
    gettimeofday(&tv,NULL);
    dim2.geo=tv;
    put(dim2);
    pthread_cond_signal(&non_empty_queue); //ksipnaei
    pthread_mutex_unlock(&for_queue_producer); //kselidono
  }  
